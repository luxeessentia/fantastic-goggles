  ...
  useEffect(()=> {
    const unsub = onAuthStateChanged(auth, async (u)=>{
      ...
      if(u){
        ...
        } catch(e){ console.warn('Firestore orders not available', e.message || e) }
      }
    });
    return ()=> unsub();
  },[]);
  ...
  return (
    <Layout>
      ...
      <section style={{marginTop:20}}>
        <h2>Orders</h2>
        {orders.length===0 ? <p>No orders yet.</p> : <ul>{orders.map(o=>(<li key={o.id}>Order #{o.id} — {o.status || 'Pending'} — </li>))}</ul>}
      </section>
    </Layout>
  );
}